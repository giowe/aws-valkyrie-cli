  ...
  fn: ({commands}) => new Promise((resolve, reject) => {
    ...
    selectScaffolder()
      .then(({scaffolder, scaffolderSourcePath, handler, root, scaffolderInputs}) => {
        ...
        vars.scaffolderSourcePath = scaffolderSourcePath;
      ...
      .then(() => {
        ...
        return Promise.all(vars.template.environments.map(env => generateRetryFn(() => createEnvRole(vars.iam, vars.template.projectName, env))()));
      })
      .then(results => {
      ...
      .then(() => {
        ...
        return Promise.all(vars.template.environments.map(env => {
          return generateRetryFn(() => createEnvPolicy(vars.iam, vars.template.projectName, env, valkconfig.Environments[env].Iam.RoleName))();
        }));
      })
      .then(results =>  {
