...
e.create = (projectName, projectFolder, lambdaTemplate, env, profile) => {
  ...
  // ROLE CREATION
    .then(() => createRole(RoleName, `Valkyrie "${projectName}" project ${env} role assumed by "${projectName}-${env}"`, basicAssumeRolePolicy, `/valkyrie/${env}/`, awsCredentials))
    ...
    .then(() => {
      ...
      return createPolicy(`${projectName}-${env}-lambda`, `Valkyrie "${projectName}" project ${env} policy attached to "${valkconfig.Iam.RoleName}"`, basicLambdaPolicy, `/valkyrie/${env}/`, awsCredentials)
    })
